...
public class lemonade {
    ...
    public static void main(String[] args) {
        ...
        // Dummy day to process last actual day that ingredients (separately) 
		// are bought.
        schedule[numDays] = new day(0,0,0);

        // Calculate cost of lemons - set up all necessary variables for day 1.
        int curPrice = schedule[0].costLemon, 
		    curLemons = lemonsPerCup * schedule[0].numCups, 
			res = 0;

        // Go through all future days - update anytime we get a better 
		// lemon price.
        for (int i = 1;  i <= numDays;  i++) {

            // We will buy on day i - process cost and reset future 
			// lemon cost and number of lemons.
        ...
        for (int i = 1;  i <= numDays;  i++) {
            ...
            if (schedule[i].costBagSugar < curPrice) {
                ...
                // This is strange - sometimes we'll need to buy no bags of 
				// sugar due to previous leftover.
                int bagsNeeded = curOzSugarNeed > 0 ? 
				     (curOzSugarNeed + OZ_SUGAR_PER_BAG - 1) / OZ_SUGAR_PER_BAG : 0;

                // These updates are straightforward.
                res += bagsNeeded * curPrice;
                curPrice = schedule[i].costBagSugar;

                // This one is strange - frequently this will be negative, 
				// indicating we have a surplus of sugar for right now.
